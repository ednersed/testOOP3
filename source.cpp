...
HeavyTank::HeavyTank(int InitX, int InitY)
    : Tank(InitX, InitY, HEAVY_BODY_WIDTH, HEAVY_BODY_HEIGHT,
           HEAVY_TURRET_WIDTH, HEAVY_TURRET_HEIGHT,
           HEAVY_GUN_LENGTH, HEAVY_TRACK_HEIGHT, HEAVY_SPEED)
{
    health = 150;
    targetX = InitX;
    targetY = InitY - 300;
...
LightTank::LightTank(int InitX, int InitY)
    : Tank(InitX, InitY, LIGHT_BODY_WIDTH, LIGHT_BODY_HEIGHT,
           LIGHT_TURRET_WIDTH, LIGHT_TURRET_HEIGHT,
           LIGHT_GUN_LENGTH, LIGHT_TRACK_HEIGHT, LIGHT_SPEED)
{
    health = 60;
}
LightTank::~LightTank() {}
...
DamageTank::DamageTank(int InitX, int InitY)
    : Tank(InitX, InitY, DAMAGE_BODY_WIDTH, DAMAGE_BODY_HEIGHT,
           DAMAGE_TURRET_WIDTH, DAMAGE_TURRET_HEIGHT,
           DAMAGE_GUN_LENGTH, DAMAGE_TRACK_HEIGHT, DAMAGE_SPEED)
{
    health = 120;
}
...
Rocket::Rocket(int InitX, int InitY, int targetX, int targetY, int pwr)
    : Point(InitX, InitY), power(pwr), isActive(true)
{
    ...
    if (distance > 0) {
        directionX = (int)(ROCKET_SPEED * deltaX / distance);
        directionY = (int)(ROCKET_SPEED * deltaY / distance);
    } else {
        directionX = ROCKET_SPEED;
        directionY = 0;
    }
}
Rocket::~Rocket() {}
...
void Rocket::Show()
{
    ...
    Ellipse(hdc, X - ROCKET_WIDTH/2, Y - ROCKET_HEIGHT/2, X + ROCKET_WIDTH/2, Y + ROCKET_HEIGHT/2);
    DeleteObject(brushBody); DeleteObject(penBody);

    // Хвост ракеты
    HPEN penTail = CreatePen(PS_SOLID, 2, RGB(255, 0, 0));
    SelectObject(hdc, penTail);
    MoveToEx(hdc, X - ROCKET_WIDTH/2, Y, NULL); LineTo(hdc, X - ROCKET_WIDTH, Y);
...
void Rocket::Hide()
{
    ...
    HBRUSH blackBrush = CreateSolidBrush(RGB(0, 0, 0));
    SelectObject(hdc, blackPen); SelectObject(hdc, blackBrush);
    Rectangle(hdc, X - ROCKET_WIDTH, Y - ROCKET_HEIGHT, X + ROCKET_WIDTH, Y + ROCKET_HEIGHT);
    DeleteObject(blackBrush); DeleteObject(blackPen);
}
...
Mina::Mina(int InitX, int InitY, int dmg)
    : Point(InitX, InitY), damage(dmg), isExploded(false) {}
Mina::~Mina() {}
...
void Mina::Show()
{
    ...
    SelectObject(hdc, pen); SelectObject(hdc, brush);
    Ellipse(hdc, X - MINA_WIDTH/2, Y - MINA_HEIGHT/2, X + MINA_WIDTH/2, Y + MINA_HEIGHT/2);
    DeleteObject(brush);

    // Шипы мины
    MoveToEx(hdc, X - MINA_WIDTH/2, Y, NULL); LineTo(hdc, X - MINA_WIDTH/2 - 5, Y);
    MoveToEx(hdc, X + MINA_WIDTH/2, Y, NULL); LineTo(hdc, X + MINA_WIDTH/2 + 5, Y);
    MoveToEx(hdc, X, Y - MINA_HEIGHT/2, NULL); LineTo(hdc, X, Y - MINA_HEIGHT/2 - 5);
    MoveToEx(hdc, X, Y + MINA_HEIGHT/2, NULL); LineTo(hdc, X, Y + MINA_HEIGHT/2 + 5);
...
void Mina::Hide()
{
    ...
    HBRUSH blackBrush = CreateSolidBrush(RGB(0, 0, 0));
    SelectObject(hdc, blackPen); SelectObject(hdc, blackBrush);
    Rectangle(hdc, X - MINA_WIDTH, Y - MINA_HEIGHT, X + MINA_WIDTH, Y + MINA_HEIGHT);
    DeleteObject(blackBrush); DeleteObject(blackPen);
}
...
Remont::Remont(int InitX, int InitY, int heal)
    : Point(InitX, InitY), healAmount(heal), isUsed(false) {}
Remont::~Remont() {}
...
void Remont::Show()
{
    ...
    SelectObject(hdc, pen); SelectObject(hdc, brush);
    Rectangle(hdc, X - REMONT_WIDTH/2, Y - REMONT_HEIGHT/2,
              X + REMONT_WIDTH/2, Y + REMONT_HEIGHT/2);
    DeleteObject(brush);
...
void Remont::Hide()
{
    ...
    HBRUSH blackBrush = CreateSolidBrush(RGB(0, 0, 0));
    SelectObject(hdc, blackPen); SelectObject(hdc, blackBrush);
    Rectangle(hdc, X - REMONT_WIDTH, Y - REMONT_HEIGHT,
              X + REMONT_WIDTH, Y + REMONT_HEIGHT);
    DeleteObject(blackBrush); DeleteObject(blackPen);
}
