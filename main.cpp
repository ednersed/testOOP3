...
// ‘ункци€ проверки столкновений с минами
...
// ‘ункци€ проверки столкновений с ремкомплектами
...
int main()
{
    ...
    if (hwnd != NULL)
    {
        ...
        if (hdc != 0)
        {
            ...
            // —оздаем мины и ремкомплекты в центре на равном рассто€нии
            const int mineCount = 4;
            Mina* mines[mineCount];
            mines[0] = new Mina(800, 600);   // центр-лево
            mines[1] = new Mina(1000, 600);  // центр
            mines[2] = new Mina(1200, 600);  // центр-право
            mines[3] = new Mina(900, 750);   // снизу центр

            const int repairCount = 3;
            Remont* repairs[repairCount];
            repairs[0] = new Remont(850, 450);   // сверху-лево
            repairs[1] = new Remont(1050, 450);  // сверху-центр
            repairs[2] = new Remont(1250, 450);  // сверху-право

            // ѕоказываем мины и ремкомплекты
            ...
            // —оздаем все танки на равном рассто€нии
            Tank mainTank(700, 800, 250, 120, 120, 70, 150, 40);
            mainTank.Show();

            HeavyTank heavy(2000, 1000);   // левее
            heavy.Show();

            LightTank light(900, 200);   // центр
            light.Show();

            DamageTank damage(1200, 200); // правее
            damage.Show();
